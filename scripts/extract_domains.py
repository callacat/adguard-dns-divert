...
import logging
import datetime
from extract_domains import download_file, extract_domains_from_file, read_custom_domains, read_dns_servers
...
def main():
    ...
    cn_dns = read_dns_servers(os.path.join('config', 'cn_dns.txt'), default_cn_dns)
    foreign_dns = read_dns_servers(os.path.join('config', 'foreign_dns.txt'), default_foreign_dns)
